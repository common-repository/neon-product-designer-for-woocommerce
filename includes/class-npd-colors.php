...
class NPD_Color {
    ...
    public function get_colors_palette_metabox() {
        ...
        foreach ($screens as $screen) {
            ...
            add_meta_box(
                    'npd-colors-palette-box', __('Colors palettes', 'neon-product-designer'), array($this, 'get_colors_palettes_page'), $screen
            );
        }
    }
    ...
    public function get_colors_palettes_page() {
        ...
        ?>
        <input type="hidden" name="npd_colors_nonce" value="<?php echo esc_html(wp_create_nonce('npd_colors_nonce')); ?>"/>
        <script>
        ...
        if ( isset( $_POST['npd_colors_nonce'] ) ) {
            if ( wp_verify_nonce( wp_unslash( sanitize_key( $_POST['npd_colors_nonce'] ) ), 'npd_colors_nonce' ) ) {
                if ( isset( $_POST['npd-colors-palette-data'] ) ) {
                    update_post_meta( $post_id, 'npd-colors-palette-data', wp_unslash( $_POST['npd-colors-palette-data'] ) );
                }
            }
        }
    }
