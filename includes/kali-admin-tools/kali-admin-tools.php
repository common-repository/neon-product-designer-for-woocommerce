<?php
/**
 * The library responsible for the UI of all Ahime products.
 *
 * @link    https://vertimcoders.com
 * @since   1.0.0
 * @author  Vertim Coders
 * @package Kali_Admin_Tools
 */
class Kali_Admin_Tools {
	 ...
	 * @return void
	 */
	public static function register_custom_post_type( $name, $slug, $desc = '' ) {
		$labels = array(
			'name'               => __( $name, 'neon-product-designer' ),
			'singular_name'      => __( $name, 'neon-product-designer' ),
			'add_new'            => __( 'New ' . $name, 'neon-product-designer' ),
			'add_new_item'       => __( 'New ' . $name, 'neon-product-designer' ),
			'edit_item'          => __( 'Edit ' . $name, 'neon-product-designer' ),
			'new_item'           => __( 'New ' . $name, 'neon-product-designer' ),
			'view_item'          => __( 'View group', 'neon-product-designer' ),
			'not_found'          => __( 'No ' . $name . ' found', 'neon-product-designer' ),
			'not_found_in_trash' => __( 'No  ' . $name . ' in the trash', 'neon-product-designer' ),
			'menu_name'          => __( $name, 'neon-product-designer' ),
		);
	...
	public static function get_fields( $options ) {
		...
		foreach ( $options as $value ) {
			...
			$settings_table  = self::get_parse_value( $options[0], 'table', 'metas' );
			...
			if ( ! in_array( $value['type'], $section_types, true ) & ! empty( $hierarchy ) ) {
				...
				$option_value = null;
				$saved_data   = get_option( 'kali-data' );
				if ( isset( $saved_data ) && ! empty( $saved_data ) ) {
					$saved_data   = filter_input( INPUT_POST, 'kali-data', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
					$option_value = self::get_parse_value( $saved_data, $session_key, false );
				}
				...
				if ( ! $option_value ) {
					...
					if ( 'metas' === $settings_table ) {
						...
						$saved_data[ $session_key ] = $option_value;
					} elseif ( 'options' === $settings_table ) { // Retrieve from the options.
						$option_value               = get_option( $root_key );
						$saved_data[ $session_key ] = $option_value;
					} elseif ( 'custom' === $settings_table ) {
						$option_value               = self::get_parse_value( $options[0], 'data', array() );
						$saved_data[ $session_key ] = $option_value;
					}
				}

				$session_key = $root_key . "_$post_id";
				$root_value  = self::get_parse_value( $saved_data, $session_key, false );
				if ( $root_key !== $value['name'] ) {
					$option_value = self::find_in_array_by_key( $root_value, $value['name'] );
				}

				update_option( 'kali-data', $saved_data );
			}

			$col_class = self::get_parse_value( $value, 'col_class', '' );
			...
			if ( ! in_array( $value['type'], $section_types, true ) && ! $value['ignore_desc_col'] ) {
				...
				echo wp_kses_post( "<div class='kali-desc'>" . $descrip . '</div>' );
				?>
					</td>
				<?php
			}
			...
			switch ( $value['type'] ) {
				case 'sectionbegin':
					...
					update_option( 'kali-data', array() );

					?>
						<div class="kali-wrap">
						<div id="<?php echo esc_attr( $value['id'] ); ?>" class="kali-metabox-container">
						<div class='block-form'>
						<table class="wp-list-table widefat fixed pages kaliroot">
						<tbody>
					<?php
					break;
				case 'sectionend':
				...
				case 'textarea':
					?>
						...
						<textarea name="<?php echo esc_attr( $value['name'] ); ?>"
							...
							style="<?php echo esc_attr( $value['css'] ); ?>"
							class="<?php echo esc_attr( $value['class'] ); ?>"
					<?php echo esc_attr( implode( ' ', $custom_attributes ) ); ?>><?php echo esc_textarea( $option_value ); ?></textarea>
				...
				case 'post-type':
					if ( 'post-type' === $value['type'] ) {
						...
						$value['args']['posts_per_page'] = self::get_parse_value( $value['args'], 'posts_per_page', - 1 );
						$posts                           = get_posts( $value['args'] );
						$posts_ids                       = self::get_parse_value( $value, 'first_value', array() );
					...
					?>
					   ...
					   <?php
						foreach ( $value['options'] as $key => $val ) {
							$option_custom_attributes = self::get_parse_value( $options_custom_attributes, $key, array() );
							?>
								<option
				...
				case 'checkbox':
					...
					$cb_value = self::get_parse_value( $value, 'value', false );
					if ( ! $cb_value ) {
						$cb_value = self::get_parse_value( $value, 'default', 1 );
					}
					?>
				...
				case 'image':
					$set_btn_label    = self::get_parse_value( $value, 'set', 'Set image' );
					$remove_btn_label = self::get_parse_value( $value, 'remove', 'Remove image' );

					$img_src      = '';
					$root_img_src = self::get_image_url( $option_value, false );
					if ( $root_img_src ) {
						$img_src = self::get_root_url( "/$root_img_src" );
					}
					?>
							<div class="<?php echo esc_attr( $value['class'] ); ?>">
								<button class="button kali-add-media"><?php echo esc_attr( $set_btn_label ); ?></button>
								<button class="button kali-remove-media"><?php echo esc_attr( $remove_btn_label ); ?></button>
					...
					break;
				case 'file':
					$set_btn_label    = self::get_parse_value( $value, 'set', 'Set file' );
					$remove_btn_label = self::get_parse_value( $value, 'remove', 'Remove file' );
					?>
							<div class="<?php echo esc_attr( $value['class'] ); ?>">
								<button class="button kali-add-media"><?php echo esc_attr( $set_btn_label ); ?></button>
								<button class="button kali-remove-media"><?php echo esc_attr( $remove_btn_label ); ?></button>
				...
				case 'repeatable-fields':
					...
					$value['popup'] = self::get_parse_value( $value, 'popup', false );
					$lazy_mode      = self::get_parse_value( $value, 'lazyload', false );
					...
					if ( $value['popup'] ) {
						...
						echo wp_kses_post( "<a class='kali-modal-trigger button button-primary button-large $modal_trigger_class' data-toggle='kali-modal' data-target='#$modal_id' data-modalid='$modal_id'>" . $value['popup_button'] . '</a>' );
						echo wp_kses_post(
							'<div class="kali-modal fade kali-modal" id="' . $modal_id . '" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
									<div class="kali-modal-dialog">
									<div class="kali-modal-content">
									<div class="kali-modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
									<h4 class="kali-modal-title" id="myModalLabel' . $modal_id . '">' . $value['popup_title'] . '</h4>
									</div>
									<div class="kali-modal-body">'
						);
						$value['class'] .= ' table-fixed-layout';
					}
					if ( $lazy_mode ) {
						$spinner_gif_url = self::get_parse_value( $value, 'spinner_gif', false );
						echo wp_kses_post(
							sprintf(
								"<img id='$modal_id-spinner' src='%s' style='display: none;'>",
								$spinner_gif_url
							)
						);
					}
					...
					foreach ( $value['fields'] as $field ) {
						...
						$col_class = self::get_parse_value( $field, 'col_class', '' );
					...
					self::get_repeatable_rows( $value, $option_value );
					$row_tpl                  = self::get_template_row( $value );
					if(!is_null($row_tpl)){
						$row_tpl                  = preg_replace( "/\r|\n/", '', $row_tpl );
						$row_tpl                  = preg_replace( '/\s+/', ' ', $row_tpl );
						$tpl_id                   = uniqid();
						$kali_tmp_rows[ $tpl_id ] = $row_tpl;
					}
					$add_label = self::get_parse_value( $value, 'add_btn_label', __( 'Add', 'npd' ) );
					?>
							</tbody>
						</table>
						<a class="button mg-top kali-add-new-row"
				...
				case 'groupedfields':
					...
					foreach ( $value['fields'] as $field ) {
						...
						echo wp_kses( self::get_fields( array( $field ) ), self::get_allowed_tags() );
					}
					?>
						</div>
				...
				default:
					dkali_action( 'kali_admin_field_' . $value['type'], $value );
					break;
			}
			if ( ! in_array( $value['type'], $section_types, true ) ) {
				if ( isset( $value['show_as_label'] ) ) {
	 ...
	 * @return string
	 */
	public static function get_template_row( $value ) {
		$row_class = self::get_parse_value( $value, 'row_class', '' );
		$row_tpl   = "<tr class='kali-rf-row $row_class'>";
		...
		foreach ( $value['fields'] as $field ) {
			...
			$row_tpl                     .= self::get_fields( array( $field_tpl ) );
		}
	  ...
	  * @return void
	  */
	public static function get_repeatable_rows( $repeatable_field_settings, $repeatable_fields_data ) {
	 ...
	 * @return mixed
	 */
	public static function get_parse_value( $values, $search_key, $default_value = '' ) {
	 ...
	 */
	public static function get_image_url( $suspected_link, $with_root = true ) {
		...
		if ( is_numeric( $suspected_link ) ) {
			...
			$img_src     = str_replace( self::get_root_url( '/' ), '', $raw_img_src );
		}
		$img_src = str_replace( self::get_root_url( '/' ), '', $img_src );
		// Code for bad https handling.
		if ( strpos( self::get_root_url( '/' ), 'https' ) === false ) {
			$https_home = str_replace( 'http', 'https', self::get_root_url( '/' ) );
			$img_src    = str_replace( $https_home, '', $img_src );
		}

		if ( $with_root ) {
			$img_src = self::get_root_url( "/$img_src" );
		}
	...
	public static function get_google_fonts_selector( $selected_font = false, $id = '', $name = '', $style = '', $class = '' ) {
		...
		$fonts_json_file = fopen( $file_path, 'r' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fopen
		$font_content    = fread( $fonts_json_file, filesize( $file_path ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fread
		fclose( $fonts_json_file ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fclose
		$decoded_fonts = json_decode( $font_content, true );
		?>
			<select id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $name ); ?>"
					style="<?php echo esc_attr( $style ); ?>"
					class="kali-google-font-selector <?php echo esc_attr( $class ); ?>">
		<?php
	 ...
	 * @return string|void
	 */
	public static function get_root_url( $path = '/' ) {
	 ...
	 * @return void
	 */
	public static function register_font( $font_name, $raw_url ) {
	...
	public static function get_allowed_tags() {
		...
		$allowed_tags['label'] = $allowed_tags['input'];
		...
		return apply_filters( 'kali_allowed_tags', $allowed_tags );
	}
}
